...
Metro=function(nsim, mu.init, delta, dataList){
  ...
  mu.samples=mu.init #원래 burn-in을 하지만 초기값 분석을 위해
  mu.curr=mu.init
  
  for (iter in 1:nsim){
    mu.prop = rnorm(1, mean=mu.curr, sd=delta)
    alpha=min (1, post.kernel(mu.prop,dataList) #채택 확률
...
mu1.codaSamples=mcmc.list(list(mu1.Samples[,1],mu1.Samples[,2],mu1.Samples[,3])) #coda패키지용 데이터로 변환 
gelman.plot(mu1.codaSamples, col=c("black", "blue")) #coda 패키지 내 함수
