setwd('~/Documents/2019_GRADUATE/Bayesian')
...
#### Figure 4.7 예 4.3에서 경로그림과 사후밀도 함수 ####
par(mfrow=c(2,2))
...
plot(density(mu.samples[,1]), type="l", xlab=quote(mu), ylab="posterior density", 
...
for(ich in 1:nChains) {
  initsList=inits.random(x)
  mcmc.samples[,,ich]=Metropolis_normal_mu_sigsq_thin(
    nsim,nburn,nthin=10,delta,dataList,initsList)
}
